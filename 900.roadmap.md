## RoadMap
  ...
  - ~~Configuration file to store various settings~~ DONE! (initial approach is sourcing)
    ...
    - Browser Specific Options and Increased Flexability
      - Consider an option / enviroment varable which will add additional --options to be passed onto the browser
      - Expose the currently selected browser (if specified via CLI) to the confguration file in a documented and hopefully somewhat thought out approach?
