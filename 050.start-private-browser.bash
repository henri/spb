...
if [[ "${help_wanted}" == "yes" ]] ; then
    ...
    echo "         Usage Summary : "
    echo ""
    echo "            # open a new private browser instance - optionally pass in URL(s)"
    echo "            $ start-private-browser [URL-TO-OPEN] [URL-TO-OPEN] ..."
    echo ""
    echo "            # show SPB help"
    echo "            $ start-private-browser --help | less -S"
    echo ""
    echo "            # show SPB version information"
    echo "            $ start-private-browser --version"
    echo ""
    ...
    echo "            $ start-private-browser --standard [URL-TO-OPEN] [URL-TO-OPEN] ..."
    echo ""
    ...
    echo ""
    echo "            # pass additional argument(s) (proxy in this example)"
    echo "            #   note : setting your proxy this way works with brave - this appraoch may not work in other browsers"
    echo "            #          browser specific infomration : https://github.com/henri/spb/blob/main/901.notes_browser_specific.md"
...
if [[ "${quiet_mode}" != "true" ]] ; then
    if [[ "${verbose_mode}" == "true" ]] ; then
        ...
        if [ -r ${spb_configuration_file_absolute} ] ; then
        echo "Configuration found : $(realpath ${spb_configuration_file_absolute})"
        fi
    fi
fi
