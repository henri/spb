...
The technical way this is implemented is that with the response from the server, there are headers set which instruct your browser to store a cookie. An example below illustrates a how web server response asks the web browser to store five individual cookies. The data for each cookie is within the header. When the browser echos back the cookie in a request it only sends back the data gram of the cookie not the associated meta data (eg. ```SameSite=Lax```). The browser makes use of cookie meta data flags in order to manage each cookie.
