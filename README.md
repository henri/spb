...
Have you wished there was an easy way to interact with the web from your terminal? Maybe like me you feel ready  to automate your web surfing from the tranquility of the shell? The bundled [SPB fish snippits](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#fish-shell-wrappers) makes searching for information from the fish shell easy-as! 
...
<p align="center">
  <a href="https://github.com/henri/spb/#lock-spb-start-private-browser"><img src="https://private-user-images.githubusercontent.com/30340/447008876-ebf7f754-655b-46c0-b3c4-a296607d5aee.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..rNY9YUg01M-A5p1rZpfa1zHJprYBRkI1JA1p7ypKp9Y" width="90%"></a>
</p>
...
[Harness the power](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#raising_hand-why-is-spb-needed) of isolated web sessions with SPB. Every time you run SPB a new instance of [Brave](https://brave.com/) (a modern web browser) is spawned within a fresh [screen](https://www.gnu.org/software/screen/) session. When you close the browser, SPB will delete all <b>web</b> data related to that browser session from your computer. 

This project is still in a [seedling stage](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#warning-disclaimer). If you find a bug please [start a discussion](https://github.com/henri/henri/discussions/categories/spb). SPB is free software, if you enjoy using SPB then share a link with friends. This way you and your friends enjoy the [benifits](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#raising_hand-why-is-spb-needed) of SPB together!
...
> <sub>Only run code from sources whom you [trust](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#lock-security-and-trust). Inspect SPB source-code [ [installer](./34f5452525ddc3727bb66729114ca8b4#file-500-spb-install-script-bash), [project](./34f5452525ddc3727bb66729114ca8b4#file-050-start-private-browser-bash) and [fish snippits](https://gist.github.com/henri/4f034f04b35c01e089e98350c902bda8) ] below.</sub>

> <sub>The [installer/update](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-500-spb-install-script-bash) script is a convienince to simplify installation and updating of spb on your system(s). Updates are semi-automatic because in order to update you manually run the [installer](./34f5452525ddc3727bb66729114ca8b4#file-500-spb-install-script-bash) interactivly. Check the [automated notes](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#arrows_counterclockwise-automate-installation-and-updates) for details on configuring unattended spb updates and installations. Also, before enabling automatic updtes read the [disclaimer section](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#warning-disclaimer) carfully.</sub>

> <sub>If you prefer to use spb without using the install/update script (not reccomdned) ; copy the SBP [project](./34f5452525ddc3727bb66729114ca8b4#file-050-start-private-browser-bash) script to your system and make it executable.<sub>
...
> <sub>During [installation](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#floppy_disk-installation) if fish is installed, you will be prompted to automatically install these fish snippits. If you install fish after SPB and would like to install these snippits, just run the [update command](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#bookmark_tabs-usage) or [re-install](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#floppy_disk-installation)</sub>
...
| ------------------------------| -------------- |
| `spb --new-template template-name` | Creates a new template and loads a browser <br>within which you may edit this newley<br>created template.Upon quit<br>the template will be saved  | 
| `spb --edit--template template-name`  | Edits the existing template and load a<br> browser in which you may edit the<br> exiting template.<br>Upon quit the template will be updated  | 
| `spb --template template-name` | Loads the existing template within a new browser.<br>Upon quit all data from<br>the session is deleted.  |
...
When automatiaclly or manually running the install / update commands, data from your system is sent to GitHub infrasturcture so that SPB is able to be installed / updated on your system. If this is a problem in your enviroment, it is suggested that you [roll your own](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#roll-your-own) update system.
...
The included [spb-update.bash](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-600-spb-update-bash) script may be run interactivly (see [usage above](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#bookmark_tabs-usage)). Alterativly, you may prefer to start it via a scheduling program (eg.[cron](https://en.wikipedia.org/wiki/Cron)) so that SPB regularly updates automatically. 
  
#### Setup Automated Updates
If you would like to have SPB update automatically, then the [spb-add-to-user-crontab.bash](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-700-spb-add-to-user-crontab-bash) script will load a pre-defined crontab entry which will automatically update SPB each week using the [spb-update.bash](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-600-spb-update-bash) script. The step of running this auto-setup cron script is a manual. The setup of a cron job is not automated in any way beyond manually starting the [spb-add-to-user-crontab.bash](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-700-spb-add-to-user-crontab-bash) script. The easiest way to setup the crontab entry is to run the command below from a bash shell :
...
The setup of unattended / automated SPB updates (eg. using the command above) will result in default choices being selected which you would normally be able to interactivly select. Also, see [notes regarding telemitry and privacy](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#shield-telemetry-and-privacy) if you schedule or manually start an SPB update.
...
If you are looking to roll your own update system, then the notes below will assist you :
  - Pick a scheduling system (cron, launchd etc..) and add an entry
    ...
    - Use the [kick off script](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#floppy_disk-installation) above to start the install / update. 
  
All of the above and much more is handled for you if you use the included [update script](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#file-600-spb-update-bash). 
...
The use cases for this script are immense and include [simplified day-to-day web browsing](https://gist.github.com/henri/34f5452525ddc3727bb66729114ca8b4#star-usage-examples), trouble-shooting, testing, load-testing, AI Agents performing multiple tasks or just having multiple tabs which are not related in terms of cookies etc. 
