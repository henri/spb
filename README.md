...
[Harness the power](https://github.com/henri/spb/blob/main/README.md#raising_hand-why-is-spb-needed) of [isolated web sessions](https://github.com/henri/spb/blob/main/README.md#sailboat-what-are-isolated-web-sessions) with SPB. Every time you run SPB a new instance of [Brave](https://brave.com/) (a modern web browser) or another browser (which you [specify](https://github.com/henri/spb/blob/main/README.md#sunrise-browser-support)) is spawned within a fresh [screen](https://www.gnu.org/software/screen/) session. When you close that web browser, the default action is for [SPB to delete all <b>web</b> data related to that browser session](https://github.com/henri/spb/blob/main/README.md#shield-privacy) from your computer.
...
SPB features built-in support for a growing list of [operating systems](https://github.com/henri/spb/blob/main/README.md#triangular_flag_on_post-compatibility) and [browsers](https://github.com/henri/spb/blob/main/README.md#sunrise-browser-support). SPB works from most interactive shells. However, keep in mind that SPB development is heavily focused towards tight [fish shell](https://fishshell.com) integration.

<br/>

More than just software. This project also attempts to be an extensive reference for anyone interested in learning more about web browser basics ; in particular the focus of this projects documentation is related to privacy as it relates to web browsers.
...
### :dart: SPB Feature Summary
  ...
  - Generate [disposable](https://github.com/henri/spb/blob/main/README.md#bookmark_tabs-usage) browser sessions
...
If you want to further isolate these sessions from one and other, run SPB within remote machine(s), virtual machines, [name spaces](https://github.com/henri/spb/tree/main/350.extras), jails, sandboxes and more. Availability of isolation technology is sometimes operating system specific.
...
#### Isolation of Browser Tasks

Using the [SPB templating system](https://github.com/henri/spb/blob/main/README.md#spb-templating-support) it is possible to have the data preserved between sessions. As an example you may create a templates for specific browsing tasks :
...
Information available via the links below will assist you with forming a deeper understanding of SPB : 
  ...
  - [Tor Network Access](https://github.com/henri/spb/blob/main/README.md#tor-network-access) <sub>:melon: onion routing</sub>
...
The technical way this is implemented is that with the response from the server, there are headers set which instruct your browser to store a cookie. An example below illustrates a how web server response asks the web browser to store five individual cookies. The data for each cookie is within the header (contained within the response sent from the server). When the browser echos back the cookie in a request it only sends back the data gram of the cookie not the associated meta data (eg. ```SameSite=Lax```). The browser makes use of cookie meta data flags in order to manage a cookie.

```
HTTP/1.1 200 OK
Set-Cookie: session=abc123; HttpOnly; Secure; SameSite=Lax
Set-Cookie: user_id=456; HttpOnly; Secure; Max-Age=2592000
Set-Cookie: theme=dark; Max-Age=31536000
Set-Cookie: cart=xyz789; SameSite=Strict
Set-Cookie: region=eu-west; Max-Age=86400
```
Typically, a web browser will processes each ```Set-Cookie``` header independently. Once stored they are able to be included with subsequent requests made to that domain (again there are edge cases and this is a simplification).
...
| `spb-rotten-tomatoes`     |  [Rotten Tomatoes Search](https://github.com/henri/spb-fish-snippits/blob/main/spb-rotten-tomatoes.fish) | initiate simultaneous [Rotten Tomatoes](https://www.rottentomatoes.com/) searches |
...
<sub>Once the SPB dependencies have been installed, [proceed with the installation of SPB](https://github.com/henri/spb/blob/main/README.md#floppy_disk-installation)</sub>
  ...
  - <b>GNU/LINUX</b>
   ...
   - <b>MacOS</b>
       ...
       - Additional dependiences provide additional tools which improve the way SPB works (they are reccomended but not required).
         - Multi-browser support in SPB requires BASH version 4 or later.
...
When automatically or manually running the [install / update command](https://github.com/henri/spb/blob/main/README.md#floppy_disk-installation), data from your system is sent to GitHub infrastructure so that SPB is able to be installed / updated on your system. If that level of telemetry is problematic in your environment, then it is suggested you [roll your own](https://github.com/henri/spb/blob/main/README.md#roll-your-own) custom update system.

As the default browser option in SPB is [Brave](https://brave.com/) and that project is a telemetry sender ; it is advisable that you read the Brave [privacy policy](https://brave.com/privacy/browser/). Brave is an [open source project](https://github.com/brave/brave-browser). However, if having any kind of telemetry being sent out is a problem then you should opt to use a different browser. Also, follow and chime into [this issue on github](https://github.com/brave/brave-browser/issues/48604) with your opinion(s) regarding telemetry opt-out features within Brave. Various workarounds exit some of these some are outlined within the [SPB FAQ](https://github.com/henri/spb/blob/main/FAQ.md#preventing-and-limiting-telemetry-data).
...
When starting a private session within the [Safari WebBrowser](https://en.wikipedia.org/wiki/Safari_(web_browser)) on [MacOS](https://en.wikipedia.org/wiki/MacOS) each tab is separate (the cookies are not shared between tabs). However, most other browsers share the private cookies with all private tabs. One approach is not better or worse. But if you would like to have multiple instances of a browser but not have them sharing all those dirty cookies, then this script allows you to start up as many private (and separate) sessions as you need (as long as your system will cope with the additional system resources being used). Have fun loading some private browsers.
  ...
  - Looking to [learn more about SPB features](https://github.com/henri/spb/blob/main/README.md#dart-spb-feature-summary)? 
...
The example command showen below instructs SPB to load Chromium rather than the Brave (the default) : 
```
...
Brave was slected as the deafult browser for the following reasons :
  ...
  - Attempts to offer the following protection by default
    - Tracker Blocking
    - Cookie Tracking
    - **[Fingerprinting Prevention](https://github.com/brave/brave-browser/wiki/Fingerprinting-Protections)**
